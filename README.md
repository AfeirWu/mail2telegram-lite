...
- **纯粹且安全**：无复杂的第三方依赖，代码极其精简，数据直接存入你的个人 CF KV 数据库（可选）。
- **直观的阅读体验**：在 Telegram 消息气泡内直接展示纯文字排版预览。
- **完美 HTML 渲染**（需绑定 KV）：点击消息底部的按钮，即可在自定义域名下丝滑查看完整邮件排版（7天自动阅后即焚清理）。
...
绑定后支持「查看完整邮件内容」网页预览功能。未绑定则仅推送 Telegram 消息，不提供网页预览。
